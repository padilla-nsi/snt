...
\section{Le routage sur Internet}\label{le-routage-sur-internet}}
    ...
    Comment un e-mail ne comportant que l'adresse de l'\textbf{expéditeur}
et du \textbf{destinataire} fait-il pour circuler sur Internet\ldots. et
arriver à destination ?

Nous allons essayer de comprendre cela !

    \hypertarget{uxe9tapes-du-protocole}{%
\subsection{1. Étapes du protocole}\label{uxe9tapes-du-protocole}}
\begin{retenir}
    \textbf{Étape 1.} Chaque élève choisit un \emph{identifiant} à 3
chiffres. Cet identifiant est \textbf{secret} et doit être différent
pour chaque élève.

        \end{retenir}\begin{retenir}
    \textbf{Étape 2.} Chaque élève crée sa \textbf{table de routage}
personnelle en indiquant sur la feuille son \emph{prénom} en première
ligne suivie de \texttt{(par\ -)}

        \end{retenir}\begin{retenir}
    \textbf{Étape 3.} Chaque élève :

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \emph{fait une copie} de sa table de routage (sur une autre feuille),
\item
  ajoute son numéro secret d'expéditeur dans l'en-tête,
\item
  transmet \emph{la copie} à l'élève à qui il est \emph{connecté}.
\end{enumerate}

        \end{retenir}\begin{retenir}
    \textbf{Étape 4.} Lorsqu'un élève reçoit la table de routage d'un de ses
nœuds, il effectue le traitement suivant :

\begin{itemize}
\tightlist
\item
  Comparer la table reçue avec sa table personnelle.
\item
  Si un \emph{nouveau prénom} apparaît, ajouter une ligne dans la table
  personnelle en indiquant :
  \texttt{\textless{}nouveau\ prénom\textgreater{}\ (par\ \textless{}identifiant\ secret\ de\ expéditeur\textgreater{})}.
\item
  Supprimer la table reçue (pour la poubelle de recyclage SVP).
\end{itemize}

        \end{retenir}\begin{retenir}
    \textbf{Étape 5.} Si la table de routage a été modifiée, l'élève :

\begin{itemize}
\tightlist
\item
  fait la liste des expéditeurs avec qui il est connecté,
\item
  fait autant de copie de sa table de routage qu'il y a d'expéditeur,
\item
  envoie une copie de sa table à chaque expéditeur.
\end{itemize}

Ensuite, l'élève reprend en boucle son attente à l'\textbf{étape 4}.
